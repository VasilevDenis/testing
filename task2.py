...
def get_shortest_and_longest_course(courses, mentors, durations):
    ...
    for id in minis:
        courses_min.append(courses_list[id][
                               'title'])  # допишите код, который берет по id нужный курс из courses_list и получает название курса из ключа "title"
    for id in maxes:
        courses_max.append(
    ...
    result = f'Самый короткий курс(ы): {", ".join(courses_min)} - {minimum} месяца(ев)'
    result += f'Самый длинный курс(ы): {", ".join(courses_max)} - {maximum} месяца(ев)'
    return result
